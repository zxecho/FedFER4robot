...
class CK(data.Dataset):
    ...
    def __init__(self, h5file, split='training', transform=None):
...
def get_fed_fam_dataset(args, data_name, split='training'):
    if 'training' == split:
        ...
        transform = transforms.Compose([
            ...
            transforms.Lambda(
                lambda crops: torch.stack([transforms.Normalize(mean=args.dataset_mean_std[args.dataset]['mean'],
                                                                std=args.dataset_mean_std[args.dataset]['std'])(
    ...
    for fm_name in fm_keys:
        dataset = CK(h5fdata, split=fm_name, transform=transform)
        if 'training' == split:
