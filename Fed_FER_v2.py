...
def fed_main(args, save_path=''):
    ...
    if args.use_teacher:
        ...
        # Evaluation on test result with PreModel
        FER_eval_acc, family_Nets_list = FERevaluateV2(args, FedNet, testset_loader_list, save_path,
                                                       'FedNet', epoch=-1)
        FER_eval_acc = FER_eval_acc.numpy().item()
        save2json(save_path, {'TestAcc': family_Nets_list}, 'PreModel_results')
    ...
    if args.idpt_usrs_training:
        ...
        for idp_N, train_dataset, test_dataset, f_name in zip(family_Nets_list, trainset_loader_list, testset_loader_list, f_n_list):
...
if __name__ == "__main__":
    ...
    # 'robot_FE_dataset'
    params_test_list = ['robot_FE_family_dataset']    # 用于设置不同参数
    ...
    for param in params_test_list:
        ...
        args.exp_name = 'PreTrained_{}_{}_Test'.format(args.dataset, args.model, param)
        ex_params_settings = {
            'algo_name': 'FedFER',
